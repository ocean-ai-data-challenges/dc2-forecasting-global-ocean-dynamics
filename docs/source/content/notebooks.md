...
4. **Visualising** existing results (GloNet baseline RMSD by lead time and depth)

Best suited for newcomers who want to understand the DC2 pipeline before
...
End-to-end notebook for evaluating **your own model** and submitting results to
the leaderboard. It covers:

1. **Preparing predictions** — a customisable data-loading block with examples
...
4. **Running the full evaluation** — creates a custom YAML config and launches
   ...
   SARAL, Jason-3, SWOT)
5. **Analysing results** — RMSD tables, lead-time curves, depth profiles,
