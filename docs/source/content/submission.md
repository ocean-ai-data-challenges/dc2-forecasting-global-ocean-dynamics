...
This page specifies the exact format requirements for a DC2 submission.
...
The `python dc2/submit.py info` command lists supported formats. Four layouts are recognised:
...
```bash
python dc2/submit.py validate <data_path> --model-name <MODEL_NAME> [options]
```
...
> The current procedure involves running `python dc2/evaluate.py` (or
> `python dc2/submit.py run`) locally and manually sending the results to the organisers. Open a
